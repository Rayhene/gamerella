...
public class player : MonoBehaviour
{
    ...
    public void levar_dano(int dano)
    {
        if (blockInput == false)
        {
            print("entrou no range de dano?");
            healthSystem.Damage(dano);
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.tag == "porta" && GameData.chave == true)
        {
            ...
            GameData.vida = 100 - healthSystem.GetHealth();
